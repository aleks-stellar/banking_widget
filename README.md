...
2. widget - в модуле реализованы две функции:
...
3. processing - в модуле реализованы функции для обработки транзакций
по дате и статусу операции:
- filter_by_state (принимает список словарей
с банковскими операциями и проводит его фильтрацию по ключу);
- get_date_format (принимает словарь с информацией о банковской операции 
и извлекает из него дату для дальнейшей сортировки);
- sort_by_date (принимает список словарей с информацией о банковской
операции и сортирует его по дате);
- filter_transactions_by_pattern (выбирает из списка словарей с
банковскими операциями те, у которых в описании есть данная строка). 
4. generators - в модуле реализованы функции для работы с массивами
транзакций. 
5. decorators - в модуле реализован декоратор log, который регистрирует
детали выполнения функции. 
6. external_api - в модуле реализована функция convert_currency
для конвертации валют в RUB с помощью Exchange Rates Data API. 
7. utils - в модуле реализована функция get_list_operations, возвращающая
список словарей с данными о финансовых транзакциях из json-файла. 
8. transactions_parser - в модуле реализованы функции:
...
в список словарей.
9. main - отвечает за основную логику проекта с пользователем и 
